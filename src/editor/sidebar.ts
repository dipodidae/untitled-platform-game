...
import { BRUSHES, BRUSH_CATEGORY_LABEL } from './brushes'
...
function toolSection(state: EditorState): HTMLElement {
  ...
  for (const t of TOOLS) {
    ...
    b.onclick = () => { state.tool = t.id; state.polyBuffer = null; markDirty(state) }
    grid.appendChild(b)
  }
  s.appendChild(grid)
  return s
...
function createSection(state: EditorState): HTMLElement {
  ...
  sel.onchange = () => { state.createMaterial = sel.value as MaterialName; markDirty(state) }
