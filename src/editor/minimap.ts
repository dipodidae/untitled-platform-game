// Minimap — tiny top-down overview of the level, drawn onto a 2D canvas
// for simplicity. The viewport rectangle shows what's currently framed in
// the main editor. Clicking on the minimap teleports the camera there.
// This matters for BIG levels where scrolling with a mouse alone is slow.

import type { EditorState } from './state'
import { markDirty } from './state'

const MATERIAL_COLORS: Record<string, string> = {
  bone: '#C8B080',
  bone_fragile: '#A08050',
  glass: '#7AC5D8',
  resonant: '#C070C0',
  soft: '#8A6EC0',
}

export function mountMinimap(
  host: HTMLElement,
  state: EditorState,
  viewport: () => { w: number, h: number },
): void {
  const canvas = document.createElement('canvas')
  canvas.width = host.clientWidth * (window.devicePixelRatio ?? 1)
  canvas.height = host.clientHeight * (window.devicePixelRatio ?? 1)
  canvas.style.width = `${host.clientWidth}px`
  canvas.style.height = `${host.clientHeight}px`
  host.appendChild(canvas)
  const ctx2d = canvas.getContext('2d')!

  const redraw = () => {
    const dpr = window.devicePixelRatio ?? 1
    const w = host.clientWidth
    const h = host.clientHeight
    if (canvas.width !== w * dpr) {
      canvas.width = w * dpr
      canvas.height = h * dpr
      canvas.style.width = `${w}px`
      canvas.style.height = `${h}px`
    }
    ctx2d.setTransform(dpr, 0, 0, dpr, 0, 0)
    ctx2d.fillStyle = '#0a0b0f'
    ctx2d.fillRect(0, 0, w, h)

    const level = state.level
    const sx = w / level.worldWidth
    const sy = h / level.worldHeight
    const s = Math.min(sx, sy)
    const offX = (w - level.worldWidth * s) / 2
    const offY = (h - level.worldHeight * s) / 2
    const wx = (x: number) => offX + x * s
    const wy = (y: number) => offY + y * s

    ctx2d.fillStyle = '#161820'
    ctx2d.fillRect(wx(0), wy(0), level.worldWidth * s, level.worldHeight * s)

    for (const c of level.colliders) {
      ctx2d.beginPath()
      const verts = c.vertices
      if (!verts.length)
        continue
      ctx2d.moveTo(wx(verts[0]![0]), wy(verts[0]![1]))
      for (let i = 1; i < verts.length; i++) ctx2d.lineTo(wx(verts[i]![0]), wy(verts[i]![1]))
      ctx2d.closePath()
      ctx2d.fillStyle = MATERIAL_COLORS[c.material] ?? '#888'
      ctx2d.globalAlpha = c.oneWay ? 0.5 : 0.9
      ctx2d.fill()
      ctx2d.globalAlpha = 1
    }

    // Spawn + markers.
    drawDot(ctx2d, wx(level.spawn.x), wy(level.spawn.y), '#40ff60')
    for (const p of level.pickups) drawDot(ctx2d, wx(p.x), wy(p.y), '#ff6040')
    for (const p of level.prowlers) drawDot(ctx2d, wx(p.x), wy(p.y), '#c040ff')
    for (const d of level.dummies) drawDot(ctx2d, wx(d.x), wy(d.y), '#ffa040')

    // Viewport rect.
    const vp = viewport()
    const halfW = vp.w / 2 / state.camera.zoom
    const halfH = vp.h / 2 / state.camera.zoom
    ctx2d.strokeStyle = '#80e0ff'
    ctx2d.lineWidth = 1
    ctx2d.strokeRect(
      wx(state.camera.x - halfW),
      wy(state.camera.y - halfH),
      halfW * 2 * s,
      halfH * 2 * s,
    )
  }

  state.listeners.add(redraw)
  redraw()

  // Click / drag to teleport camera.
  let dragging = false
  const onMove = (e: MouseEvent) => {
    if (!dragging && e.type === 'mousemove')
      return
    const rect = canvas.getBoundingClientRect()
    const x = e.clientX - rect.left
    const y = e.clientY - rect.top
    const level = state.level
    const w = rect.width
    const h = rect.height
    const sx = w / level.worldWidth
    const sy = h / level.worldHeight
    const s = Math.min(sx, sy)
    const offX = (w - level.worldWidth * s) / 2
    const offY = (h - level.worldHeight * s) / 2
    state.camera.x = (x - offX) / s
    state.camera.y = (y - offY) / s
    markDirty(state)
  }
  canvas.addEventListener('mousedown', (e) => { dragging = true; onMove(e) })
  canvas.addEventListener('mousemove', onMove)
  window.addEventListener('mouseup', () => { dragging = false })
}

function drawDot(c: CanvasRenderingContext2D, x: number, y: number, color: string): void {
  c.fillStyle = color
  c.beginPath()
  c.arc(x, y, 2, 0, Math.PI * 2)
  c.fill()
}
