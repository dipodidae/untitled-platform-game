...
import { resetPlayerRenderer } from './playerRenderer'
...
import { drawColliders, hashColliders } from './world'
...
export interface RenderContext {
  ...
  hintSeen: { moved: boolean, jumped: boolean }
  hintAlpha: number
...
export function buildScene(app: Application, level: Level, particles: ParticleSystem): RenderContext {
  ...
  const hint = new Text({
    text: '← → / A D  move    SPACE  jump    R  begin again',
  ...
  return {
    ...
    ruptureFrame: -1,
    respawnFrame: -1,
    hintSeen: { moved: false, jumped: false },
    hintAlpha: 0.55,
    hpGlowTimer: 0,
    armorGlowTimer: 0,
    pickupFlashTimer: 0,
...
export function render(
  ...
  const allSeen = ctx.hintSeen.moved && ctx.hintSeen.jumped
